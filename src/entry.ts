...
async function getUID(username: string): Promise<string> {
    try {
...
async function checkIfAlreadyFriends(uid1: string, uid2: string): Promise<boolean> {
    try {
...
app.post("/api/login", async (req, res) => {
    const body: userRequestType = req.body;
...
app.get("/api/getFriends", async (req, res) => {
    ...
    try {
        ...
        friends.forEach((node) => {
            // @ts-ignore
            let innerUsername: string = node.user1!.username === username ? node.user2!.username : node.user1!.username;
